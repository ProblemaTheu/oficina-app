...
| **[plano-10-dias.md](plano-10-dias.md)** | ⏱️ **Documento de trabalho** — 1 pessoa, 01/09→10/09, ~42 h. Os 16 cortes, cronograma diário e ordem de sacrifício. **Comece por aqui.** |
| [execucao.md](execucao.md) | 📋 **Diário de execução** — o que já foi feito, valores reais do ambiente (contas, buckets, roles) e os achados técnicos que custaram tempo |
...
| **Ambiente de homologação** | **Dispensado** por orientação no fórum — só `prod` é provisionado; a branch `homolog` fica com CI, sem deploy | O PDF do enunciado pede deploy das duas branches, então **guarde o print do post e cite nos 4 READMEs**: sem a evidência, a divergência conta contra. Ver [corte 10](plano-10-dias.md#corte-nº-10--ambiente-único). |
...
| **Ligação API Gateway → EKS** | VPC Link + ALB interno + `TargetGroupBinding` — **simplificado para NLB público com header compartilhado** ([corte 1](plano-10-dias.md#os-cortes)) | O desenho com VPC Link elimina a dependência circular (o Gateway precisaria do ARN de um ALB que só existe após o deploy). Ver [F3-3.5](backlog.md#f3-35--alb-interno-target-group-e-vpc-link). |
