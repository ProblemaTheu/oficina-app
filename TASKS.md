...
- [ ] Definir transições válidas: `recebida`, `em_diagnostico`, `aguardando_aprovacao`, `em_execucao` → `cancelada`
- [ ] Adicionar endpoint `POST /v1/work-orders/{id}/cancel` no `docs/openapi.yaml`
- [ ] Adicionar schema `CancelarOSRequest` e campo `cancelado_em` em `OrdemServicoResponse`
- [ ] Regenerar código com `oapi-codegen`
- [ ] Implementar `CancelarOS` em `internal/application/usecase/ordem_servico_usecase.go`
- [ ] Implementar handler `PostWorkOrdersIdCancel` em `internal/infra/http/api/server.go`
- [ ] Restaurar estoque de peças automaticamente quando cancelado a partir de `em_execucao`
- [ ] Atualizar diagrama da máquina de estados no `README.md`
